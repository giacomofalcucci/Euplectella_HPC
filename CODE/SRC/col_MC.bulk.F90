...
!====================================================
!
        subroutine col_MC_bulk(itime,bulk)
!
        use storage
        use real_kinds
        use timing
...
!
        ...
        real(mykind) :: pi, border
!
        logical:: flag_i, flag_j, flag_k
!
#ifdef ORIGINAL
...
!
        if(bulk.eq.1) then 
!
!         write(6,*) "INFO --> collision only on bulk"
!
...
!$OMP SHARED(istart,istop,jstart,jstop,kstart,kstop)  &
...
!$OMP DO
!!!!$acc present(a02(0:l1,0:n1),a04(0:l1,0:n1),a05(0:l1,0:n1),a06(0:l1,0:n),a11(0:l1,0:n1),a13(0:l1,0:n1),a14(0:l1,0:n1),a15(0:l1,0:n1),a19(0:l1,0:n1))  
           do k = kstart,kstop
           do j = jstart,jstop
!!!!!$acc kernels loop 
!!$OMP SIMD 
...
!
           vxpy = vx+vy
           qxpy = uno+qf*(tre*vxpy*vxpy-vsq)
           vxmy = vx-vy
           qxmy = uno+qf*(tre*vxmy*vxmy-vsq)
           vxpz = vx+vz
           qxpz = uno+qf*(tre*vxpz*vxpz-vsq)
           vxmz = vx-vz
           qxmz = uno+qf*(tre*vxmz*vxmz-vsq)
           vypz = vy+vz
           qypz = uno+qf*(tre*vypz*vypz-vsq)
           vymz = vy-vz
           qymz = uno+qf*(tre*vymz*vymz-vsq)
           qx   = uno+qf*(tre*vx2-vsq)
           qy   = uno+qf*(tre*vy2-vsq)
           qz   = uno+qf*(tre*vz2-vsq)
           vx   = rf*vx
           vy   = rf*vy
           vz   = rf*vz
           vxpy = rf*vxpy
...
!
!           write(6,*) "INFO --> collision only on border"
!
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(i,j,k)  &
!$OMP PRIVATE(x01,x02,x03,x04,x05,x06,x07,x08,x09,x10)  &
...
!$OMP PRIVATE(qxpz,qxmz,qxpy,qxmy,qypz,qymz,qx,qy,qz)  &
!$OMP PRIVATE(forcex,forcey,forcez)  &
!$OMP PRIVATE(flag_i,flag_j,flag_k)  &
!$OMP SHARED(omega,fgrad,l,m,n,itime)  &
!$OMP SHARED(myrank)  &
!$OMP SHARED(istart,istop,jstart,jstop,kstart,kstop)  &
!$OMP SHARED(a01,a02,a03,a04,a05,a06,a07,a08,a09,a10)  &
...
!$OMP DO
           do k = 1,n
           flag_k= ((k.GT.kstop).OR.(k.LT.kstart))
           do j = 1,m
           flag_j= ((j.GT.jstop).OR.(j.LT.jstart))
!DIR$ IVDEP
           do i = 1,l
              flag_i= ((i.GT.istop).OR.(i.LT.istart))
              if ((flag_k).OR.(flag_j).OR.(flag_i)) then 
!
! compute border...
!
              x01 = a01(i-1,j+1,k  )
...
#ifdef DEBUG_2
        if(myrank == 0) then
           write(6,*) "DEBUG2: Exiting from sub. coll_MC_bulk", bulk
        endif
#endif
!
        return 
        end subroutine col_MC_bulk
