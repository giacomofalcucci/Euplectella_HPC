...
!=====================================================================
!
        subroutine bcond_comm_noblock_packed
!
        use timing
        use storage
#ifdef SERIAL
...
!
        call time(tcountX0)
!
! isend from front: step 1 pack data
        if((front(1) == 0).and.(front(2).NE.myrank)) then
!
...
!
! isend from rear: step 1 pack data
        if((rear(1) == 0).and.(rear(2).NE.myrank)) then
!
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,k)  &
...
!
! isend from front: step 2 send data
        if((front(1) == 0).and.(front(2).NE.myrank)) then
           tag = 01
...
! isend from rear: step 2 send data
        if((rear(1) == 0).and.(rear(2).NE.myrank)) then
           tag = 10
...
!
#ifdef TRYBOX
       call col_MC_box(0,border+1,l-border-1,border+1,m-border-1,border+1,n-border-32,0)
!       call col_MC_box(0,border+1,l-border-1,border+1,m-border-1,border+1,n-border-1,0)
#endif
!
! recv from rear: step 1, receive data
        if((rear(1) == 0).and.(rear(2).NE.myrank)) then
!
           tag = 01
...
!
! recv from front: step1, receive data
        if((front(1) == 0).and.(front(2).NE.myrank)) then
!
           tag = 10
...
!
! recv from rear: step2, unpack data
        if((rear(1) == 0).and.(rear(2).NE.myrank)) then
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,k)  &
...
!
! recv from front: step2, unpack data
        if((front(1) == 0).and.(front(2).NE.myrank)) then
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,k)  &
...
! send from right: step1, pack data
        if((right(1) == 0).and.(right(2).NE.myrank)) then
!
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(i,k)  &
...
!
! send from left: step1, pack data
        if((left(1) == 0).and.(left(2).NE.myrank)) then
!
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(i,k)  &
...
!
! send from right: step2, send data
        if((right(1) == 0).and.(right(2).NE.myrank)) then
!
           tag = 03
...
!
! send from left: step2, send data
        if((left(1) == 0).and.(left(2).NE.myrank)) then
!
           tag = 01
...
!
#ifdef TRYBOX
       call col_MC_box(0,border+1,l-border-1,border+1,m-border-1,n-border-31,n-border-1,0)
!       call col_MC_box(0,border+1,l-border-1,border+1,m-border-1,border+1,n-border-1,0)
#endif
!
! recv from left: step1, receive data
        if((left(1) == 0).and.(left(2).NE.myrank)) then
!
           tag = 03
...
!
! recv from right: step1, receive data
        if((right(1) == 0).and.(right(2).NE.myrank)) then
!
           tag = 01
...
!
! recv from left: step2, unpack data
        if((left(1) == 0).and.(left(2).NE.myrank)) then
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(i,k)  &
...
!
! recv from right: step2, unpack data
        if((right(1) == 0).and.(right(2).NE.myrank)) then
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(i,k)  &
...
!!!!CHECK  call mpi_wait(reqs_right(1), status_right, ierr)
        endif
        if((left(1) == 0).and.(left(2).NE.myrank)) then
...
! first isend 
        call time(tcountZ0)
!
! send from up: step1, pack data
        if((up(1) == 0).and.(up(2).NE.myrank)) then
!
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,i)  &
...
!
! send from down: step1, pack data
        if((down(1) == 0).and.(down(2).NE.myrank)) then
!
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,i)  &
...
!
! send from down: step2, send data
        if((down(1) == 0).and.(down(2).NE.myrank)) then
           tag = 02
...
!
! recv from up: step1, receive data
        if((up(1) == 0).and.(up(2).NE.myrank)) then
!
           tag = 02
...
!
! recv from down: step1, receive data
        if((down(1) == 0).and.(down(2).NE.myrank)) then
!
           tag = 04
...
!
! recv from up: step2, unpack data
        if((up(1) == 0).and.(up(2).NE.myrank)) then
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,i)  &
...
!
!
! recv from down: step2, unpack data
        if((down(1) == 0).and.(down(2).NE.myrank)) then
!$OMP PARALLEL DEFAULT(NONE)  &
!$OMP PRIVATE(j,i)  &
...
#ifdef DEBUG_2
        if(myrank == 0) then
           write(6,*) "DEBUG2: Exiting from sub. bcond_comm_noblock_packed"
        endif
#endif
!
        return
        end subroutine bcond_comm_noblock_packed
